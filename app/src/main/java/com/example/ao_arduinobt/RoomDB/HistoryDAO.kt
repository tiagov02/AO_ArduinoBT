...
interface HistoryDAO {
    ...
    @Query("SELECT strftime('%H:00', date_time_measure) AS hour, AVG(temperature) AS avg_temperature, AVG(humidity) AS avg_humidity FROM history GROUP BY strftime('%H', date_time_measure) ORDER BY date_time_measure ASC")
